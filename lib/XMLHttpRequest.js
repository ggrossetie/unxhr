...
exports.XMLHttpRequest = function () {
  ...
  // Other values will result in returning a Buffer (which is an Uint8Array)
  this.responseType = ''
   ...
   */
  this.open = function (method, url, async, user, password, rejectUnauthorized = true) {
    this.abort()
    errorFlag = false
      ...
      const responseHandler = function responseHandler (resp) {
        ...
        if (encoding === 'utf8') {
          ...
          response.on('end', function () {
            ...
            self.response = Buffer.concat(self.responseBinary)
            if (sendFlag) {
              // Discard the end event if the connection has been aborted
              setState(self.DONE)
              sendFlag = false
...
--request-options=${JSON.stringify(JSON.stringify(options))}`, { stdio: ['pipe', 'pipe', 'pipe'], input: data })
      ...
      if (result.error) {
        ...
        if (encoding === 'binary') {
          self.response = result.data.binary.data
        } else {
  ...
  this.handleError = function (error) {
    this.status = 0
    this.statusText = JSON.stringify(error)
